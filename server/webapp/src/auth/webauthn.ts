...
function ensureAvailable() {
  if (!webAuthnSupported()) throw new Error(tx('securityKeyUnavailable'));
}

export function useCeremonySignal(): () => AbortSignal {
  const ceremony = useRef<AbortController | null>(null);
  useEffect(() => () => ceremony.current?.abort(), []);
  return useCallback(() => {
    ceremony.current?.abort();
    ceremony.current = new AbortController();
    return ceremony.current.signal;
  }, []);
}
