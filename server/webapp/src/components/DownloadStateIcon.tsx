...
import { ConfirmDialog, toast } from './Primitives';
...
export function DownloadStateIcon({ state, onDownload, onChanged }: {
  ...
  const act = (fn: () => Promise<unknown>) => async (e: React.MouseEvent) => {
    e.stopPropagation();
    try { await fn(); } catch (err) { reportError(err); }
    onChanged();
  };
    ...
    default:
      button = (
        <button className="icon-btn primary" aria-label={t('downloads.download')} title={t('downloads.download')}
                onClick={act(async () => toast((await onDownload()).message))}>
          <Icon name="download" />
        </button>
      );
  }
    ...
    <>
      ...
      {confirmDelete && state && (
        <ConfirmDialog
          ...
          onConfirm={async () => {
            await api.deleteDownload(state.id).catch(reportError);
            onChanged();
          }}
