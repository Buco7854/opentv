...
import { Dialog, IconBtn, ScreenHeader, Spinner, TextField } from '../components/Primitives';
...
export function SessionsScreen() {
  ...
  const reconcile = (list: Session[]): Session[] => {
    const now = Date.now();
    const held = pendingPaused.current;
    const stillHeld = new Map<string, { paused: boolean; until: number }>();
    const next = list.map((s) => {
      const pending = held.get(s.id);
      if (!pending || now > pending.until || s.paused === pending.paused) return s;
      stillHeld.set(s.id, pending);
      return { ...s, paused: pending.paused };
    });
    pendingPaused.current = stillHeld;
    return next;
  };
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        // A failed poll keeps the last roster on screen. Reporting it would raise a toast
        // every few seconds for as long as the server is unreachable.
      }
  ...
  const command = async (session: Session, type: 'pause' | 'play') => {
    ...
    try {
      ...
      reportError(cause, { [GENERIC]: () => t('sessions.commandFailed') });
    }
...
function MessageDialog({ session, onDismiss }: { session: Session; onDismiss: () => void }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      onDismiss();
      reportSuccess(t('sessions.messageSent'));
    } catch (cause) {
      reportError(cause, { [GENERIC]: () => t('sessions.commandFailed') });
    }
  };
