...
class TrustedProxies(private val ranges: List<IpRange>) {
    ...
    /**
     * Whether the direct peer may speak for the client. The same trust decision gates every
     * `X-Forwarded-*` header, not just the address ones.
     */
    fun trustsPeer(call: ApplicationCall): Boolean = isTrusted(call.request.origin.remoteAddress)
