...
internal class RemuxSession(
    ...
    /** Monotonic timings span /remux/start and the later HLS fragment request. */
    val startupStartedNs: Long,
    val connectionLimitMs: Long,
    val ffprobeMs: Long,
    @Volatile var playlistPreparationMs: Long = 0,
    ...
    val firstFragmentRequestNs = AtomicLong()
    val firstLaunchStartedNs = AtomicLong()
    val firstLaunchFinishedNs = AtomicLong()
    val firstInitReadyNs = AtomicLong()
    val firstSegmentReadyNs = AtomicLong()
    val firstLaunchSegment = AtomicLong(-1)
    val startupLogged = AtomicBoolean()
