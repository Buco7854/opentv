...
class RemuxService(
    ...
    companion object {
        ...
        // Nothing is decodable until one whole fragment closes, so this is the floor on
        // time-to-first-frame. Copied video can only cut on a source keyframe, so a shorter
        // target helps exactly as far as the provider's GOP allows and costs more segments.
        private const val COPY_SEGMENT_SEC = 2
    ...
    internal fun start(
        ...
        val probeFinishedNs = System.nanoTime()
        val ffprobeMs = elapsedMs(probeStartedNs, probeFinishedNs)
    ...
    private fun launch(session: RemuxSession, startNumber: Int) {
        ...
        val launchStartedNs = System.nanoTime()
    ...
    private suspend fun produce(session: RemuxSession, n: Int, file: Path) = withContext(Dispatchers.IO) {
        session.firstFragmentRequestNs.compareAndSet(0, System.nanoTime())
        repeat(2) {
            ensureReaching(session, n)
            awaitFile(session, file)
            if (Files.exists(file)) {
                recordStartupReady(session)
                return@withContext
            }
            stopReading(session)
        }
    ...
    /** Emit one correlated record once both init and the first media fragment are usable. */
    private fun recordStartupReady(session: RemuxSession) {
        val now = System.nanoTime()
        recordInitReady(session, now)
        session.firstSegmentReadyNs.compareAndSet(0, now)
        val initReadyNs = session.firstInitReadyNs.get()
        val segmentReadyNs = session.firstSegmentReadyNs.get()
        if (initReadyNs == 0L || segmentReadyNs == 0L ||
            !session.startupLogged.compareAndSet(false, true)
        ) return
        val launchStartedNs = session.firstLaunchStartedNs.get()
        val launchFinishedNs = session.firstLaunchFinishedNs.get()
        log.debug(
            "media_start id={} connection_limit_ms={} ffprobe_ms={} playlist_ms={} " +
                "ffmpeg_launch_ms={} init_ms={} first_segment_ms={} fragment_wait_ms={} " +
                "total_ms={} segment_target_s={} first_segment={} video_mode={} audio_codec={}",
            session.id,
            session.connectionLimitMs,
            session.ffprobeMs,
            session.playlistPreparationMs,
            elapsedMs(launchStartedNs, launchFinishedNs),
            elapsedMs(launchFinishedNs, initReadyNs),
            elapsedMs(launchFinishedNs, segmentReadyNs),
            elapsedMs(session.firstFragmentRequestNs.get(), segmentReadyNs),
            elapsedMs(session.startupStartedNs, segmentReadyNs),
            session.segLenSec,
            session.firstLaunchSegment.get(),
            if (session.transcodeVideo) "transcode" else "copy",
            session.audio.codec,
        )
    }

    private fun recordInitReady(session: RemuxSession, now: Long = System.nanoTime()) {
        if (session.firstInitReadyNs.get() == 0L && hasContent(session.initFile)) {
            session.firstInitReadyNs.compareAndSet(0, now)
        }
    }
    ...
    suspend fun segment(id: String, n: Int, call: ApplicationCall) {
        ...
        if (!withContext(Dispatchers.IO) { Files.exists(segment) } &&
            !produceOrRefuse(session, n, segment, call)
        ) return
        if (!withContext(Dispatchers.IO) { Files.exists(segment) }) return failed(session, call)
        withContext(Dispatchers.IO) { pruneBehind(session, n) }
    ...
    private suspend fun respondFile(call: ApplicationCall, path: Path, type: ContentType) {
        ...
        val (size, modifiedMs) = withContext(Dispatchers.IO) {
            Files.size(path) to Files.getLastModifiedTime(path).toMillis()
        }
        val etag = "\"${shortSha1("${path.fileName}:$size:$modifiedMs")}\""
        ...
        // LocalFileContent streams from disk and lets PartialContent satisfy a seek range
        // without first allocating and reading the entire fragment.
        call.respond(LocalFileContent(path.toFile(), type))
    }
    ...
    private fun elapsedMs(startNs: Long, endNs: Long = System.nanoTime()): Long =
        if (startNs <= 0L || endNs < startNs) 0 else (endNs - startNs) / 1_000_000
}
