...
internal class AuthFlowService(
    ...
    /**
     * [relyingParty] is the WebAuthn identity of the address being asked. Passkeys are
     * offered only where they can work: on a bare IP or plain HTTP the browser refuses the
     * ceremony, so advertising the button would promise something undeliverable.
     */
    ...
    suspend fun activate(request: ActivationRequestDto, clientIp: String): AuthResult =
        mutation.withLock {
            ...
            sessions.revokeUser(user.id, clock())
    ...
    suspend fun startTotpEnrollment(
        ...
        credentials.requirePassword(user.id, "enrolling an authenticator")
        ...
        val enroll = accounts.issueChallenge(
            ...
            payload = "${pending.credential.id}|${parent.id}",
            ttlMs = 5 * 60_000L,
        )
        TotpEnrollmentDto(
