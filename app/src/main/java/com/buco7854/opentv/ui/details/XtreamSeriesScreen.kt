...
/**
 * Series page for native Xtream playlists: header data comes from the panel's
 * catalog; episodes are fetched on first open (one request, cached for a day).
 */
...
fun XtreamSeriesScreen(
    ...
    LaunchedEffect(seriesId) {
        ...
        try {
            ...
            error = "Couldn't load episodes: ${ErrorLog.describe(e)}"
        }
        loading = false
    }
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            item {
                Poster(series?.cover, Icons.Rounded.VideoLibrary)
