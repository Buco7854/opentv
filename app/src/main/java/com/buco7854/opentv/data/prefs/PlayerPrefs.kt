...
data class PlayerSettings(
    ...
    /** HTTP User-Agent override; "" = app default (VLC). Some panels 404/403 unknown agents. */
    val userAgent: String = "",
    /** THEME_SYSTEM follows the device; THEME_DARK / THEME_LIGHT force one. */
    val themeMode: Int = THEME_SYSTEM,
    /** The playlist the dock's sections act on; -1 = none yet. */
...
class PlayerPrefs(private val context: Context) {
    ...
    private object Keys {
        ...
        val ACTIVE_PLAYLIST = longPreferencesKey("active_playlist")
    }
    ...
    val settings: Flow<PlayerSettings> = context.playerDataStore.data.map { prefs ->
        PlayerSettings(
            ...
            themeMode = prefs[Keys.THEME_MODE] ?: PlayerSettings.THEME_SYSTEM,
            activePlaylistId = prefs[Keys.ACTIVE_PLAYLIST] ?: -1L,
        )
    }
    ...
    suspend fun save(settings: PlayerSettings) {
        context.playerDataStore.edit { prefs ->
            ...
            prefs[Keys.ACTIVE_PLAYLIST] = settings.activePlaylistId
        }
    }
}
