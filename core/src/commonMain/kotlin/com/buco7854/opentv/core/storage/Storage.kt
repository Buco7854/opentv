...
/** A search response is intentionally small enough for phone, TV, and browser list rendering. */
const val SEARCH_RESULTS_PER_KIND = 50
...
/** One bounded listing read and the count used to render its pager. */
...
/**
 * Columns needed to identify and render a channel in a listing.
 *
 * In particular, this deliberately omits provider metadata such as an episode description.
 * The provider URL is internal identity input and never crosses the server DTO boundary.
 */
...
interface ChannelStore {
    ...
    /**
     * Ranked substring search, bounded independently for live, movie, and series rows.
     * Adapters treat [query] as data, never as query-language syntax.
     */
    suspend fun search(
        playlistId: Long,
        query: String,
        limitPerKind: Int = SEARCH_RESULTS_PER_KIND,
    ): List<Channel>
...
interface XtreamSeriesStore {
    ...
    /** Ranked substring search bounded to [limit]; see [ChannelStore.search]. */
    suspend fun search(
        playlistId: Long,
        query: String,
        limit: Int = SEARCH_RESULTS_PER_KIND,
    ): List<XtreamSeries>
